  ...
  return (
    <>
      <nav className='flex fixed h-full text-gray-700 border-b border-gray-200 z-10'>
        <div className={`z-10 fixed inset-0 transition-opacity ${isCollapse ? 'hidden' : ''}`}>
          <div
        ...
        >
          <button
            type='button'
            className='flex h-1 px-4 items-center text-gray-500 focus:outline-none'
        ...
        >
          ...
          {sidebarItems.map(({ name, path, routes }, index) => (
            <li key={index} className='py-2'>
              ...
              <ul className='w-full pl-6 mt-2'>
                {routes.map(({ name: routeName, path: routePath }, index) => (
                  <li key={index} className='py-2'>
